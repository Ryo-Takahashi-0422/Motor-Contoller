...
となっている。  
アプリからの通信データはBluetoothデバイスを介して1bitずつMCUに送信されてくる。  
...
の示すメモリ領域にある値がSCI0.TDRに書き込まれる。(なお、SCI0.TDRの値は自動的にSCI0.TSR  
に転送され、TXD0端子を通じて送信されていく)  
続けてポインタをインクリメントし、カウンタをデクリメントする。この処理を繰り返し、カウンタが0に　　
なると送信データは全てSCI0.TDRに書き込み終えたとして、通信を完了するために送信  
割り込みの禁止、SCI送信割り込み有効化・送信完了割り込みの許可などの処理を行い、  
送信完了割り込みを発生させる。  
